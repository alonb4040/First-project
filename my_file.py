...
# adding new articles to bank_of_articles(raw data).
...
keywords = input("Please insert some keywords ").replace(",", " ").split()
name = input("\nWhat is your name?").title()
match = input('Optional: Insert 1 for partial match and 2 for full match')
while match not in ['1', '2']:
    match = input('Amit don\'t play with me, insert 1 or 2 :)')
date_of_publication = input('Optional: Insert a date of publication in the following format "year-month-day", press one for today results only. (The default it anytime)')
...
# main function
def api_news_articles(articles, keywords, match = '1'):

    msg = f'\nHi {name},\n\nHereinafter your resutls for those keywords: {str(keywords)[1:-1]}'
    ...
    if list_of_missing_words == list(keywords):
        ...
        msg += f'\n\n*** We couldn\'t find any results for:{list_of_missing_words}'
